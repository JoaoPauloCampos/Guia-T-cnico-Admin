...
<?php
	...
	echo "\t\t<li>";
	echo $this->Form->postLink(__d('cake', 'Delete %s', $singularHumanName), array('action' => 'delete', ${$singularVar}[$modelClass][$primaryKey]), array(), __d('cake', 'Tem certeza que quer deletar # %s?', ${$singularVar}[$modelClass][$primaryKey]));
	echo " </li>\n";
...
<?php
		...
		foreach (${$singularVar}[$_alias] as ${$otherSingularVar}):
			...
			echo $this->Form->postLink(
				...
				array(),
				__d('cake', 'Tem certeza que quer deletar # %s?', ${$otherSingularVar}[$_details['primaryKey']])
			);
			echo "\n";
			echo "\t\t\t</td>\n";
		echo "\t\t</tr>\n";
