...
foreach (${$pluralVar} as ${$singularVar}):
	echo '<tr>';
		...
		echo ' ' . $this->Form->postLink(
			...
			array(),
			__d('cake', 'Tem certeza que quer deletar # %s?', ${$singularVar}[$modelClass][$primaryKey])
		);
		echo '</td>';
	echo '</tr>';
...
?>
	...
	echo $this->Paginator->counter(array(
		'format' => __d('cake', 'Pagina {:page} de {:pages}, mostrando {:current} registros de {:count} total, começando em {:start}, terminando em {:end}')
	));
	?></p>
	<div class="paging">
	<?php
